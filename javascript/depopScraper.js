...
const scrapeCollectionListings = async (
	page,
	collectionObj,
	elementTargetor
) => {
	try {
		...
		listings = await container.evaluate((c) => {
			...
			return Array.from(c.querySelectorAll("li")).map((pod) => {
				...
				const sizeTarget =
					"sc-eDnWTT styles__StyledSizeText-sc-4aad5806-12 kcKICQ cuCvzt";
		...
		for (let listing of listings) {
			...
			if (
				listing.productSize?.toLowerCase() == "other" ||
				listing.productSize?.toLowerCase() == "one size"
			) {
				invalid += 1;
				continue;
			}
...
const scrapeAllCollections = async (l, u) => {
	try {
		...
		while (i < u) {
			...
			await scrapeCollectionListings(
				page,
				collectionObj,
				elementTargetor
			);
